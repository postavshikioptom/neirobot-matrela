...
class XLSTMIndicatorModel:
    ...
    def train(self, X_train, y_train, epochs=50, batch_size=32, validation_split=0.1):
        ...
        if np.any(np.isnan(X_train_scaled)) or np.any(np.isinf(X_train_scaled)):
            print("!!! ВНИМАНИЕ: NaN или Inf значения найдены после масштабирования в train")
    ...
    def predict(self, X_test):
        ...
        if np.any(np.isnan(X_test_scaled)) or np.any(np.isinf(X_test_scaled)):
            print("!!! ВНИМАНИЕ: NaN или Inf значения найдены после масштабирования в predict")
    ...
    def evaluate(self, X_test, y_test):
        ...
        if np.any(np.isnan(X_test_scaled)) or np.any(np.isinf(X_test_scaled)):
            print("!!! ВНИМАНИЕ: NaN или Inf значения найдены после масштабирования в evaluate")
            X_test_scaled = np.nan_to_num(X_test_scaled, nan=0.0, posinf=0.0, neginf=0.0)
